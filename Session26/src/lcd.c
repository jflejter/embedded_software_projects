...
#define PAGE_PIXEL_HEIGHT		8
#define LCD_PAGE_HEIGHT			(LCD_PIXEL_HEIGHT / PAGE_PIXEL_HEIGHT)
#define CHAR_PIXEL_WIDTH		6
#define OBJ_PIXEL_HEIGHT		8
#define OBJ_PIXEL_WIDTH			8
...
void LcdClear(void)
{
	...
	for (DispPage = 0; DispPage < LCD_PAGE_HEIGHT; DispPage++)
	{
		...
		{
			SpiDataWrite(0x00);
			ShadowDisp[DispPage][DispCol] = 0x00;
		}
	}
}
...
{
	static UInt8 ObjectImages[3][8] =
	{
		{0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00},	// paddle
		{0x1c, 0x3e, 0x7f, 0x7f, 0x7f, 0x3e, 0x1c, 0x00},	// ball
		{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}	// erase
	};
...
//	PosX = PosX < 0 ? 0 : PosX;
//	PosX = PosX > (LCD_PIXEL_WIDTH - OBJ_PIXEL_WIDTH) ? LCD_PIXEL_WIDTH - OBJ_PIXEL_WIDTH : PosX;
//	PosY = PosY < 0 ? 0 : PosY;
//	PosY = PosY > (LCD_PIXEL_HEIGHT - OBJ_PIXEL_HEIGHT) ? LCD_PIXEL_HEIGHT - OBJ_PIXEL_HEIGHT : PosY;

	// Determine the starting column, page, and offset within the page
	DispCol = PosX;
	DispPage = PosY / PAGE_PIXEL_HEIGHT;
	VertOffset = PosY - (DispPage *  PAGE_PIXEL_HEIGHT);
	...
	for (RenderCol = DispCol, ImageCol = 0; ImageCol < OBJ_PIXEL_WIDTH; RenderCol++, ImageCol++)
	{
		DispVal = ShadowDisp[DispPage][RenderCol];
		RenderVal = (DispVal & ByteTopMask[VertOffset]) |
					(ImageBufPtr[ImageCol] << VertOffset);
		if (Object == LCD_OBJ_ERASE)
		{
			RenderVal = DispVal;
		}
	...
	for (RenderCol = DispCol, ImageCol = 0; ImageCol < OBJ_PIXEL_WIDTH; RenderCol++, ImageCol++)
	{
		DispVal = ShadowDisp[DispPage][RenderCol];
		RenderVal = (DispVal & ~ByteTopMask[VertOffset]) |
					(ImageBufPtr[ImageCol] >> (PAGE_PIXEL_HEIGHT - VertOffset));
		if (Object == LCD_OBJ_ERASE)
		{
			RenderVal = DispVal;
		}
...
int LcdPrintf(int CharRow, int CharCol, char * FmtStrPtr, ...)
{
	...
	SpiCmdWrite(CMD_COL_UPPER + ((CharCol * CHAR_PIXEL_WIDTH) >> 4));	// RAM column address upper 4 bits
	SpiCmdWrite(CMD_COL_LOWER + ((CharCol * CHAR_PIXEL_WIDTH) & 0x0F));	// RAM column address lower 4 bits
...
static void CharWrite(int CharVal)
{
	...
	{
		SpiDataWrite(FontArray[(CharVal * CHAR_PIXEL_WIDTH) + CharCol]);
	}
}
